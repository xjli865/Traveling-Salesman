...
def print_cities(road_map):
    ...
    for line in road_map:
        a = float(line[2])
        b = float(line[3])
        print("{0:^5} {1:<14} {2:^5}{3:<14} {4:>10} {5:<7} {6:>12} {7:<5}".format("State:",
                                                                                line[0],
                                                                                "City:",
                                                                                line[1],
                                                                                "latitude:",
...
def compute_total_distance(road_map):
    ...
    for i in range(len(road_map)):
        ...
        total_distance = total_distance + distance(lat1degrees, long1degrees, lat2degrees, long2degrees)
...
def find_best_cycle(road_map):
    ...
    new_road_map = road_map[:]
    number_of_swaps = 0
    update_distance = 50000
    for i in range(0,5000):
        swap_results = swap_cities(road_map, randint(0 ,len(road_map)-1), randint(0 ,len(road_map)-1))
        number_of_swaps += 1
        if  swap_results[1] < update_distance:
            new_road_map = swap_results[0][:]
            update_distance = compute_total_distance(new_road_map)
    for i in range(0,5000):
        new_swap_results = swap_adjacent_cities(road_map, randint(0, len(road_map)-1))
        number_of_swaps += 1
...
def print_map(road_map):
    ...
    print("-"*92)
    print("{0:^14} {1:^16} {2:^1}{3:^17} {4:^15} {5:^4}{6:<9} {7:<9}".format("From state",
                                                                               "From city",
                                                                               "|",
                                                                               "To state",
                                                                               "To city",
    ...
    for i in range(len(road_map)):
        ...
        print("{0:^14} {1:^16} {2:^1}{3:^17} {4:^15} {5:^4}{6:<9} {7:<9}".format(state,
                                                                                   city,
                                                                                   "|",
                                                                                   next_state,
                                                                                   next_city,
